...
import (
	...
	"github.com/xiao-ren-wu/hertzfs"
	"html/template"
	"net/http"
	"path/filepath"
	"regexp"
...
func CustomWrapHandler(config *Config, handler *webdav.Handler) app.HandlerFunc {
	...
	index, _ := template.New("index.html").Parse(swaggerIndexTpl)
	...
	return func(ctx context.Context, c *app.RequestContext) {
		...
		matches := matcher.FindStringSubmatch(reqPath)
		...
		request, _ := http.NewRequest(string(c.Request.Method()), string(c.Request.URI().Path()), c.RequestBodyStream())
		hw := hertzfs.NewCustomerWriter(c)
		switch path {
		case "index.html":
			_ = index.Execute(hw, config.toSwaggerConfig())
		case "doc.json":
